...
str_thisfunction = 'test_interp_k_p_to_x_p_xxnufft';
...
#%%%%%%%%;
n_x_M_u = 128;
x_c_0_ = torch.linspace(-half_diameter_x_c,+half_diameter_x_c,n_x_M_u+1).to(dtype=torch.float32); x_c_0_ = x_c_0_[:-1];
x_c_1_ = torch.linspace(-half_diameter_x_c,+half_diameter_x_c,n_x_M_u+1).to(dtype=torch.float32); x_c_1_ = x_c_1_[:-1];
x_c_1__,x_c_0__ = torch.meshgrid(x_c_1_,x_c_0_,indexing='ij'); #<-- reversed to match matlab. ;
dx = x_c_0_[1].item() - x_c_0_[0].item();
n_xx_M_u = n_x_M_u**2;
...
delta_ = 3*0.75*torch.tensor([+0.1,-0.2]).to(dtype=torch.float32);
M_x_c_form_ = 1/(np.sqrt(2*pi)*sigma_x_c)**2 * torch.exp( -( (x_c_0__-delta_[0])**2 + (x_c_1__-delta_[1])**2 ) / (2*sigma_x_c**2) );
M_x_c_form_l2 = sum_0(M_x_c_form_**2)*dx**2;
disp(sprintf(' %% sum(M_x_c_form_*dx**2,''all'') = %0.16f',sum_0(M_x_c_form_*dx**2)));
disp(sprintf(' %% M_x_c_form_l2 = %0.16f',M_x_c_form_l2));
M_x_p_form_ = 1/(np.sqrt(2*pi)*sigma_x_c)**2 * torch.exp( -( (x_c_0_tx_-delta_[0])**2 + (x_c_1_tx_-delta_[1])**2 ) / (2*sigma_x_c**2) );
M_x_p_form_l2 = sum_0(torch.abs(M_x_p_form_)**2 * weight_2d_x_p_tx_) * (2*pi)**2;
disp(sprintf(' %% M_x_p_form_l2 = %0.16f',M_x_p_form_l2));
M_k_p_quad_ = interp_x_c_to_k_p_xxnufft(n_x_M_u,diameter_x_c,n_x_M_u,diameter_x_c,M_x_c_form_,n_k_p_r,k_p_r_,n_w_)*np.sqrt(n_x_M_u**2)*dx**2;
...
if (flag_verbose>1):
    darray_printf_margin(k_p_r_,1,n_k_p_r,' %% k_p_r_: '); disp(sprintf(' %%'));
    darray_printf_margin(n_w_,1,n_k_p_r,' %% n_w_: '); disp(sprintf(' %%'));
    darray_printf_margin(weight_2d_k_p_wk_,n_w_max,n_k_p_r,' %% weight_2d_k_p_wk_: '); disp(sprintf(' %%'));
    darray_printf_margin(x_p_r_,1,n_x_p_r,' %% x_p_r_: '); disp(sprintf(' %%'));
    darray_printf_margin(n_t_,1,n_x_p_r,' %% n_t_: '); disp(sprintf(' %%'));
    darray_printf_margin(weight_2d_x_p_tx_,n_t_max,n_x_p_r,' %% weight_2d_x_p_tx_: '); disp(sprintf(' %%'));
    zarray_printf_margin(M_x_p_form_,n_t_max,n_x_p_r,' %% M_x_p_form_: '); disp(sprintf(' %%'));
    zarray_printf_margin(M_k_p_reco_,n_w_max,n_k_p_r,' %% M_k_p_reco_: '); disp(sprintf(' %%'));
...
N_k_p_reco_ = xxnufft2d3(n_t_sum,x_c_0_tx_*eta,x_c_1_tx_*eta,M_x_p_form_*weight_2d_x_p_tx_,-1,1e-12,n_w_sum,2*pi*k_c_0_wk_/eta,2*pi*k_c_1_wk_/eta) * (2*pi)**2;
...
#end;%if (flag_verbose>1);
M_x_c_reco_ = interp_k_p_to_x_c_xxnufft(n_x_M_u,diameter_x_c,n_x_M_u,diameter_x_c,n_k_p_r,k_p_r_,n_w_,M_k_p_quad_*weight_2d_k_p_wk_*(2*pi)**2)*np.sqrt(n_x_M_u**2) * n_w_sum;
M_x_c_reco_l2 = sum_0(torch.abs(M_x_c_reco_)**2)*dx**2;
