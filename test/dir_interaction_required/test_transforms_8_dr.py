...
from dir_empm.I_xPPx_0 import I_xPPx_0 ;
...
for nk_p_r in range(n_k_p_r):
    ...
    for l_val in range(l_max+1):
        for m_val in range(-l_val,+l_val+1):
            ...
            tmp_a_k_Y_form_lm__ = matlab_assign_lhs_from_rhs_(tmp_a_k_Y_form_lm__,tmp_i8_index_lhs_, tmp_a_k_Y_form_lm_[na].item());
            na=na+1;
        #end;%for m_val=-l_val:+l_val;
    #end;%for l_val=0:l_max;
    ...
    a_k_Y_form_yk___ = matlab_assign_lhs_from_rhs_(a_k_Y_form_yk___,tmp_i8_index_lhs_, tmp_a_k_Y_form_lm__.ravel());
#end;%for nk_p_r=0:n_k_p_r-1;
#%%%%;
flag_check=1;
if flag_check:
...
for nS in range(n_S):
    ...
    T_k_p_wkS__ = matlab_assign_lhs_from_rhs_(T_k_p_wkS__,tmp_i8_index_lhs_, T_k_p_wk_);
#end;%for nS=0:n_S-1;
fnorm_disp(flag_verbose,'T_k_p_wkS__',T_k_p_wkS__,'S_k_p_wkS__',S_k_p_wkS__,' %%<-- should be <1e-2');
fnorm_disp(flag_verbose,'T_k_p_l2_form_S_',T_k_p_l2_form_S_,'T_k_p_l2_quad_S_',T_k_p_l2_quad_S_,' %%<-- should be <1e-2');
fnorm_disp(flag_verbose,'T_k_p_l2_form_S_',T_k_p_l2_form_S_,'S_k_p_l2_quad_S_',S_k_p_l2_quad_S_,' %%<-- should be <1e-2');
...
for nCTF in range(n_CTF):
    ...
    Z_dwSM_ampm____ = matlab_assign_lhs_from_rhs_(Z_dwSM_ampm____,tmp_i8_index_lhs_, svd_USESVUXCTFM_sub_dwSM____.ravel() );
    #%%%%;
